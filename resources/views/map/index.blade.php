...
<head>
    ...
    <style>
        body { background: linear-gradient(135deg, #f4e8ff, #e0f2ff); }
        .card-list-item:hover { background: #f9fafb; }
        
        /* Custom Scrollbar */
        #list::-webkit-scrollbar { width: 4px; }
        #list::-webkit-scrollbar-track { background: rgba(255, 255, 255, 0.1); }
        #list::-webkit-scrollbar-thumb { background: rgba(255, 255, 255, 0.5); border-radius: 10px; }
    </style>
</head>
...
<body class="font-sans relative min-h-screen flex flex-col overflow-x-hidden" style="background:#ff8c32;">
    ...
    <nav class="mx-2 lg:mx-5 mt-2 lg:mt-3 bg-orange-500/40 rounded-2xl lg:rounded-3xl backdrop-blur-md text-white px-4 py-3 shadow-lg sticky top-2 z-50 border border-white/20 transition-all">
        <div class="flex flex-col lg:flex-row lg:items-center justify-between gap-4 lg:gap-0">
            <div class="flex items-center justify-between w-full lg:w-auto">
                ...
                <button id="mobileMenuBtn" class="lg:hidden text-white/90 hover:text-white focus:outline-none p-1 bg-white/10 rounded-md">
            ...
            <div class="flex flex-wrap lg:flex-nowrap justify-center gap-2 lg:gap-3 items-center w-full lg:w-auto">
                <div class="grid grid-cols-2 lg:flex gap-2 w-full lg:w-auto">
                    <select id="filterJenjang" class="col-span-1 bg-white/20 border border-white/30 text-white text-xs lg:text-sm rounded-xl focus:outline-none focus:ring-2 focus:ring-white/50 py-2 px-3 cursor-pointer">
                    ...
                    <select id="filterStatus" class="col-span-1 bg-white/20 border border-white/30 text-white text-xs lg:text-sm rounded-xl focus:outline-none focus:ring-2 focus:ring-white/50 py-2 px-3 cursor-pointer">
                ...
                <select id="filterAkreditasi" class="w-full lg:w-auto bg-white/20 border border-white/30 text-white text-xs lg:text-sm rounded-xl focus:outline-none focus:ring-2 focus:ring-white/50 py-2 px-3 cursor-pointer">
                ...
                <div class="relative group w-full lg:w-64">
                    ...
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 absolute left-3 top-3 text-white/80" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            ...
            <div class="hidden lg:flex gap-6 text-lg font-medium ml-4">
                <a href="#" class="hover:text-yellow-200 transition">Peta</a>
                <a href="#" class="hover:text-yellow-200 transition">Tentang</a>
            </div>
        </div>

        <div id="mobileMenu" class="hidden lg:hidden flex-col gap-2 mt-4 pb-2 border-t border-white/20 pt-4">
            <a href="#" class="block px-4 py-2 hover:bg-white/10 rounded-lg transition font-medium">📍 Peta</a>
            <a href="#" class="block px-4 py-2 hover:bg-white/10 rounded-lg transition font-medium">ℹ️ Tentang</a>
        </div>
    </nav>
        ...
        <div class="lg:p-6">
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-4 lg:gap-6">
                ...
                <div class="col-span-1 lg:col-span-2 relative order-1 lg:order-2">
                    <div id="map" class="w-full h-[50vh] lg:h-full min-h-[300px] rounded-2xl shadow-lg border border-white/20 z-0"></div>
                </div>

                <div class="col-span-1 order-2 lg:order-1 bg-white/15 backdrop-blur-md rounded-2xl p-4 h-[45vh] lg:h-[calc(100vh-140px)] shadow-inner flex flex-col border border-white/20">
                    <div class="flex justify-between items-center pb-2 border-b border-white/20 mb-2">
                        ...
                        <span id="countDisplay" class="text-[10px] lg:text-xs bg-orange-600 text-white px-2 py-1 rounded-full shadow">0 Data</span>
                    </div>
    ...
    <footer class="mt-8 bg-orange-900/40 backdrop-blur-xl border-t border-white/10 text-white pt-10 pb-6">
        <div class="max-w-7xl mx-auto px-6 lg:px-8">
            <div class="grid grid-cols-1 md:grid-cols-3 gap-8 mb-8">
                ...
                <div>
                    ...
                    <p class="text-white/70 text-sm leading-relaxed">
                        Sistem Informasi Geografis untuk pemetaan lokasi sekolah di Kabupaten Pringsewu. Temukan informasi lengkap sekolah mulai dari SD, SMP, hingga SMA/SMK.
                    </p>
                </div>
                ...
                <div>
                    ...
                    <ul class="space-y-2 text-sm text-white/80">
                        <li><a href="#" class="hover:text-white transition hover:underline">Beranda</a></li>
                        <li><a href="#" class="hover:text-white transition hover:underline">Peta Persebaran</a></li>
                        <li><a href="#" class="hover:text-white transition hover:underline">Data Sekolah</a></li>
                        <li><a href="#" class="hover:text-white transition hover:underline">Tentang Aplikasi</a></li>
                    </ul>
                </div>
            ...
            <div class="border-t border-white/10 pt-6 flex flex-col md:flex-row justify-between items-center gap-4 text-xs text-white/50">
    ...
    <script>
        ...
        function renderData(items, limitList = null) {
            ...
            if(items.length === 0) {
                listContainer.innerHTML = '<div class="text-center text-white/80 p-4 italic text-sm">Data tidak ditemukan.</div>';
                return;
            }
            ...
            listItems.forEach(item => {
                ...
                div.className = 'p-3 lg:p-4 rounded-xl border bg-white shadow-sm cursor-pointer hover:bg-orange-50 transition transform active:scale-95 duration-200';
                div.innerHTML = `
                ...
                div.onclick = () => {
                    ...
                    document.querySelectorAll('#list > div').forEach(el => el.classList.remove('ring-2', 'ring-orange-500'));
            ...
            .then(data => {
                ...
                L.geoJSON(geojsonData, {
                    onEachFeature: (feature, layer) => {
                        ...
                        let content = `<b class="text-orange-600 text-lg">${feature.properties.NAMA_SEKOLAH}</b><br>`;
                        content += `<div class="text-sm mt-1 text-gray-600">${feature.properties.ALAMAT}</div>`;
